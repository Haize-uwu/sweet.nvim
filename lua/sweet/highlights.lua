local c = require('sweet.colors')
local cfg = vim.g.sweet_config
local util = require("sweet.util")
...
function M.setup()
        ...
        if color_name:sub(1, 1) == '$' then
            ...
            if not color_name then
                vim.schedule(function()
                    vim.notify('sweet.nvim: unknown color "' .. name .. '"', vim.log.levels.ERROR, { title = "sweet.nvim" })
                end)
                return ""
            end
        end
    ...
    for group_name, group_settings in pairs(vim.g.sweet_config.highlights) do
